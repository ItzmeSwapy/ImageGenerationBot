        ...
        if int(check1):
            ...
            for x in musicPathFiles:
                for y in m2:
                    ...
                    try:
                        os.system(
                            f"""ffmpeg -i {y1} -i {x1} -map 0:v -map 1:a -c:v copy -shortest {outputPath}/{fname}.mp4 -loglevel quiet""")
                    except:
                        pass
            ...
            for y in mainPathFiles:
                for x in imagePathFiles:
                    ...
                    imagePathExactFile = f"tempimg/{x}"
                    filename = get_random_string(10)
                    q = "ffmpeg -i {} -i {} -filter_complex \"[0:v][1:v] overlay=(W-w)/2:(H-h)/{}\" -pix_fmt yuv420p -c:a copy merged/{}.mp4 -loglevel quiet".format(
