...
def refactoring_func(dial, image_dimensions, remove_audio):
    ...
    '''for ind, ele in enumerate(os.listdir(mainPath)):
        ...
        if remove_audio:
            q = f'ffmpeg -i {dest} -ar 44100 -vf \"scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2,setsar=1\" -an {FINAL}'

        else:
            q = f'ffmpeg -i {dest}- ar 44100 -vf \"scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2,setsar=1\" {FINAL}'
